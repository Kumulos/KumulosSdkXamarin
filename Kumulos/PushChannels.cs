...
namespace Kumulos
{
    public class PushChannels
    {
		...
		{
            var uri = new Uri(string.Format("https://push.kumulos.com/v1/app-installs/{0}/channels", KumulosSDK.InstallId));
			
			HttpResponseMessage request = await KumulosSDK.GetHttpClient().GetAsync(uri);
		...
		public async Task<object> CreateChannel(string uuid, bool subscribe, string name, bool showInPortal, Dictionary<string, object> meta)
		{
			...
			if (subscribe == true)
			{
                payload.Add("installId", KumulosSDK.InstallId);
			}
			...
			HttpResponseMessage request = await KumulosSDK.GetHttpClient().SendAsync(requestMessage);
        ...
        private async Task<object> MakeSubscriptionNetworkRequest(HttpMethod method, string payload) {
            var uri = new Uri(string.Format("https://push.kumulos.com/v1/app-installs/{0}/channels/subscriptions", KumulosSDK.InstallId));
